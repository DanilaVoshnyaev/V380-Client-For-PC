...
`auto` определяет вашу сеть сам — не нужно узнавать IP и разбираться, что
такое подсеть. В собранной версии двойной клик по **scan-cameras.bat** делает
всё то же самое: обходит сеть, создаёт отчёт и открывает его, без единой
команды. Сканирование адреса за пределами вашей сети запрещено, пока не
передан `--i-own-this`.

### Отчёт, который можно отдать

    python probe.py --scan 192.168.1.0/24 --report-html otchet.html

Складывает обход в одну самодостаточную HTML-страницу: сводка, таблица
устройств, рабочие адреса потоков и предупреждение по каждой камере, которая
отдаёт видео без пароля. Наружу страница не ходит ни за чем, поэтому
открывается без интернета, с телефона, с флешки и нормально печатается в PDF.

В отчёте адреса и MAC реального оборудования — в репозиторий его класть
не нужно.
...
При первом запуске клиент откроет окно подключения: нажмите **Найти камеры** —
он найдёт их в вашей сети сам, знать IP не нужно, — выберите камеру, введите
логин и пароль и подключитесь. Выбор сохраняется, в следующий раз подключение
происходит сразу. Открыть окно заново можно кнопкой **Подключение…**.
...
- Просмотр с телефона — кнопка **Смотреть на телефоне…** показывает QR-код
  со ссылкой на поток: наведите камеру телефона и откройте в VLC. Работает
  в той же сети Wi-Fi; из другой сети нужен VPN, см. [vpn/](vpn/README.md)
...
Проект бесплатный и таким останется. Если он сэкономил вам вечер — кошелёк
в [DONATE.md](DONATE.md). Это необязательно, и взамен ничего не причитается.

Добавить свою камеру в базу полезнее денег и стоит одной команды.
